...
class TranceKickGenerator:
    ...
    def generate_punch(self, phase_deg=0.0):
        ...
        # Optimization: Active duration processing
        # Calculate samples needed for punch + release
        ...
        # 1.0 to 0.5 over decay
        amp_env[:decay_samples] = np.linspace(1.0, 0.5, decay_samples)

        # 0.5 to 0.0 over release
        remaining = total_len - decay_samples
        if remaining > 0:
        ...
        full_signal[:total_len] = active_signal
    ...
    def generate_body(self, phase_deg=0.0):
        ...
        # Pad to full duration
        full_signal = np.zeros(self.num_samples)
        full_signal[:decay_samples] = active_signal
    ...
    def generate_click(self, level=1.0, decay_ms=10.0, width=0.0):
        ...
        # Optimization: Generate noise only for active duration
        if width > 0.001:
            # Stereo Noise via Mid/Side
        ...
        active_signal = filtered_noise * amp_env * level

        # Pad to full duration
        if active_signal.ndim == 2:

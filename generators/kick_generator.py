...
class TranceKickGenerator:
    ...
    def generate_punch(self, phase_deg=0.0):
        ...
        # Bolt Optimization: Only compute signal for active duration
        ...
        # Time array for active region
        t_active = np.arange(total_len) / self.sample_rate

        # Frequency Envelope
        freq_env_active = np.full(total_len, base_freq)

        # Apply pitch drop for decay portion
        # Ensure we don't index past total_len if decay_samples > total_len
        # (unlikely unless truncated)
        eff_decay = min(decay_samples, total_len)
        t_decay = t_active[:eff_decay]

        # Exponential interpolation
        freq_env_active[:eff_decay] = start_freq * (
            (base_freq / start_freq) ** (t_decay / punch_decay)
        )

        # Generate phase by integrating frequency
        phase_active = (
            2 * np.pi * np.cumsum(freq_env_active) / self.sample_rate
        )

        # Add start phase offset
        phase_offset = phase_deg * np.pi / 180.0
        signal_active = np.sin(phase_active + phase_offset)

        # Amplitude Envelope
        amp_env_active = np.zeros(total_len)

        # 1.0 to 0.5
        amp_env_active[:eff_decay] = np.linspace(1.0, 0.5, eff_decay)

        # 0.5 to 0.0
        if total_len > eff_decay:
            len_release = total_len - eff_decay
            amp_env_active[eff_decay:] = np.linspace(0.5, 0.0, len_release)

        result_active = signal_active * amp_env_active

        # Pad to full length
        result = np.zeros(self.num_samples)
        result[:total_len] = result_active
    ...
    def generate_body(self, phase_deg=0.0):
        ...
        # Bolt Optimization: Only compute for decay duration
        ...
        result = np.zeros(self.num_samples)
        result[:decay_samples] = result_active
    ...
    def generate_click(self, level=1.0, decay_ms=10.0, width=0.0):
        ...
        # Bolt Optimization: Generate noise only for active duration
        # Strict cut at decay_time implies we don't need tail
        process_samples = decay_samples

        # Generate white noise (Stereo if width > 0)
        if width > 0.001:
            mid = np.random.uniform(-1, 1, process_samples)
            side = np.random.uniform(-1, 1, process_samples) * width
            ...
            noise = np.random.uniform(-1, 1, process_samples)
        ...
        # Envelope
        t_active = np.arange(process_samples) / self.sample_rate

        # Exponential decay for crispness
        k = 7.0 / decay_time
        amp_env_active = np.exp(-k * t_active)

        result_active = filtered_noise * amp_env_active * level

        # Pad to full length
        if noise.ndim == 2:
            result = np.zeros((2, self.num_samples))
            result[:, :process_samples] = result_active
        else:
            result = np.zeros(self.num_samples)
            result[:process_samples] = result_active

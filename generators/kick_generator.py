...
class TranceKickGenerator:
    ...
    def generate_punch(self, phase_deg=0.0):
        ...
        # During decay
        decay_mask = t_active < punch_decay
        t_decay = t_active[decay_mask]
        freq_envelope[decay_mask] = start_freq * ((base_freq / start_freq) ** (t_decay / punch_decay))

        # After decay (release)
        freq_envelope[~decay_mask] = base_freq

        # Phase
        phase = 2 * np.pi * np.cumsum(freq_envelope) / self.sample_rate
        phase_offset = phase_deg * np.pi / 180.0
        signal_short = np.sin(phase + phase_offset)

        # Amp Env
        amp_env_short = np.zeros(total_len)
        amp_env_short[:decay_samples] = np.linspace(1.0, 0.5, decay_samples)

        # Release phase
        release_len = min(release_samples, total_len - decay_samples)
        if release_len > 0:
             amp_env_short[decay_samples:decay_samples+release_len] = np.linspace(0.5, 0.0, release_len)

        # Apply envelope
        signal_short = signal_short * amp_env_short

        # Pad to full duration
        signal = np.zeros(self.num_samples)
        signal[:total_len] = signal_short
    ...
    def generate_body(self, phase_deg=0.0):
        ...
        # Optimization: Generate only active duration
        decay_samples = int(decay_time * self.sample_rate)
        if decay_samples > self.num_samples:
            decay_samples = self.num_samples

        # Time array for active duration
        t_active = np.arange(decay_samples) / self.sample_rate

        # Signal
        phase_offset = phase_deg * np.pi / 180.0
        signal_short = np.sin(2 * np.pi * body_freq * t_active + phase_offset)
        ...
        # Pad to full duration
        signal = np.zeros(self.num_samples)
        signal[:decay_samples] = signal_short
    ...
    def generate_click(self, level=1.0, decay_ms=10.0, width=0.0):
        ...
        # Generate short envelope
        t_active = np.arange(active_samples) / self.sample_rate
        k = 7.0 / decay_time
        amp_env_short = np.exp(-k * t_active)

        # Apply envelope
        result_short = filtered_noise * amp_env_short * level

        # Pad to full length
        if noise.ndim == 2:
            result = np.zeros((2, self.num_samples))
            result[:, :active_samples] = result_short
        else:

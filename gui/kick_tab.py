...
class KickTab(ttk.Frame):
    def __init__(self, parent, main_window):
        ...
        ttk.Label(q_frame, text="(1=Std, 2=High/Oversampled)").pack(
            side=tk.LEFT, padx=5
        )

        self.create_slider(
            synth_frame, "Duration (s)", 0.1, 1.0, 0.5, "duration",
            tooltip_text="Length of the kick."
        )
        self.create_slider(
            synth_frame, "Start Phase (deg)", 0.0, 360.0, 0.0, "phase",
            tooltip_text="Adjusts the starting phase alignment of the "
                         "sub/punch."
        )
        self.create_slider(
            synth_frame, "Click/Noise Level", 0.0, 3.0, 1.0, "click_level",
            tooltip_text="Volume of the white noise click layer."
        )
        self.create_slider(
            synth_frame, "Click Decay (ms)", 5.0, 100.0, 10.0, "click_decay",
            tooltip_text="Length of the click envelope (5-15ms typical)."
        )
        self.create_slider(
            synth_frame, "Click Width (Stereo)", 0.0, 2.0, 0.0, "click_width",
            tooltip_text="Stereo width for the click layer "
                         "(0=Mono, 1=Normal, 2=Wide)."
        )
        ...
        self.create_slider(
            ...
            tooltip_text="Amount of short, dense reverb."
        )
        self.create_slider(
            fx_frame, "Delay Amount", 0.0, 1.0, 0.0, "delay",
            tooltip_text="Amount of stereo ping-pong delay."
        )
        ...
        self.create_slider(
            ...
            tooltip_text="Pitch of the bassline."
        )
        # Sidechain Depth
        self.create_slider(
            bass_frame, "Sidechain Depth", 0.0, 1.0, 0.8, "sc_depth",
            tooltip_text="How much the bass volume drops when the kick hits."
        )

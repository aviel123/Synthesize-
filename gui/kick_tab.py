...
class KickTab(ttk.Frame):
    def __init__(self, parent, main_window):
        ...
        self.create_slider(synth_frame, "Duration (s)", 0.1, 1.0, 0.5, "duration", "Length of the kick drum in seconds")
        self.create_slider(synth_frame, "Start Phase (deg)", 0.0, 360.0, 0.0, "phase", "Phase alignment of the sub/punch (0-360 deg)")
        self.create_slider(synth_frame, "Click/Noise Level", 0.0, 3.0, 1.0, "click_level", "Volume of the high-frequency transient attack")
        self.create_slider(synth_frame, "Click Decay (ms)", 5.0, 100.0, 10.0, "click_decay", "Length of the transient click (short = snappy)")
        self.create_slider(synth_frame, "Click Width (Stereo)", 0.0, 2.0, 0.0, "click_width", "Stereo width of the click (0=mono, 2=super-wide)")

        # --- Effects Tab ---
        self.create_slider(fx_frame, "Saturation Drive (dB)", 0.0, 12.0, 4.5, "drive", "Harmonic distortion amount (warmth/grit)")
        self.create_slider(fx_frame, "Reverb Amount", 0.0, 1.0, 0.0, "reverb", "Mix level of the reverb tail")
        self.create_slider(fx_frame, "Delay Amount", 0.0, 1.0, 0.0, "delay", "Mix level of the stereo ping-pong delay")
        ...
        # Bass Freq
        self.create_slider(bass_frame, "Bass Frequency (Hz)", 30.0, 100.0, 55.0, "bass_freq", "Fundamental frequency of the offbeat bassline")
        # Sidechain Depth
        self.create_slider(bass_frame, "Sidechain Depth", 0.0, 1.0, 0.8, "sc_depth", "How much the bass ducks when the kick hits")

...
class KickTab(ttk.Frame):
    def __init__(self, parent, main_window):
        ...
        self.create_slider(synth_frame, "Duration (s)", 0.1, 1.0, 0.5, "duration", "Total length of the kick in seconds")
        self.create_slider(synth_frame, "Start Phase (deg)", 0.0, 360.0, 0.0, "phase", "Phase offset for the sub/punch layers")
        self.create_slider(synth_frame, "Click/Noise Level", 0.0, 3.0, 1.0, "click_level", "Volume of the high-frequency click/noise layer")
        self.create_slider(synth_frame, "Click Decay (ms)", 5.0, 100.0, 10.0, "click_decay", "Duration of the click envelope (5-15ms recommended)")
        self.create_slider(synth_frame, "Click Width (Stereo)", 0.0, 2.0, 0.0, "click_width", "Stereo spread of the click (0=Mono, 2=Wide)")

        # --- Effects Tab ---
        self.create_slider(fx_frame, "Saturation Drive (dB)", 0.0, 12.0, 4.5, "drive", "Soft-clipping distortion amount for harmonics")
        self.create_slider(fx_frame, "Reverb Amount", 0.0, 1.0, 0.0, "reverb", "Level of the short, dense room reverb")
        self.create_slider(fx_frame, "Delay Amount", 0.0, 1.0, 0.0, "delay", "Level of the stereo ping-pong delay")
        ...
        # Bass Freq
        self.create_slider(bass_frame, "Bass Frequency (Hz)", 30.0, 100.0, 55.0, "bass_freq", "Fundamental frequency of the accompanying bassline")
        # Sidechain Depth
        self.create_slider(bass_frame, "Sidechain Depth", 0.0, 1.0, 0.8, "sc_depth", "Amount of volume reduction on the bass when the kick hits")

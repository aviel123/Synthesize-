...
class KickTab(ttk.Frame):
    def __init__(self, parent, main_window):
        ...
        self.create_slider(
            ...
            "Total length of the kick sample in seconds."
        )
        self.create_slider(
            synth_frame, "Start Phase (deg)", 0.0, 360.0, 0.0, "phase",
            "Initial phase of the oscillator. 0-360 degrees."
        )
        self.create_slider(
            synth_frame, "Click/Noise Level", 0.0, 3.0, 1.0, "click_level",
            "Volume of the high-frequency click/noise layer."
        )
        self.create_slider(
            synth_frame, "Click Decay (ms)", 5.0, 100.0, 10.0, "click_decay",
            "How fast the click layer fades out."
        )
        self.create_slider(
            synth_frame, "Click Width (Stereo)", 0.0, 2.0, 0.0, "click_width",
            "Stereo width of the click layer (0=Mono, 2=Wide)."
        )

        # --- Effects Tab ---
        self.create_slider(
            fx_frame, "Saturation Drive (dB)", 0.0, 12.0, 4.5, "drive",
            "Amount of distortion/saturation applied to the kick."
        )
        self.create_slider(
            fx_frame, "Reverb Amount", 0.0, 1.0, 0.0, "reverb",
            "Level of the reverb effect tail."
        )
        self.create_slider(
            fx_frame, "Delay Amount", 0.0, 1.0, 0.0, "delay",
            "Level of the delay/echo effect."
        )

        # --- Bassline Tab ---
        bass_frame = ttk.LabelFrame(
            bass_frame_tab,
            text="Bassline & Sidechain (VST/DAW Integration)", padding="10"
        )
        ...
        self.create_slider(
            ...
            "Frequency of the accompanying bassline note."
        )
        # Sidechain Depth
        self.create_slider(
            bass_frame, "Sidechain Depth", 0.0, 1.0, 0.8, "sc_depth",
            "Amount of volume reduction on the bass when the kick hits."
        )

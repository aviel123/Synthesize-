...
class KickTab(ttk.Frame):
    def __init__(self, parent, main_window):
        ...
        q_combo = ttk.Combobox(
            q_frame, textvariable=self.vars["oversample"], values=[1, 2],
        ...
        self.create_slider(
            ...
            "Total length of the kick sample. Typical trance kicks "
            "are 0.3-0.5s."
        )
        self.create_slider(
            synth_frame, "Start Phase (deg)", 0.0, 360.0, 0.0, "phase",
            "Starting phase of the sub-bass sine wave. Adjust to align "
            "with bassline."
        )
        self.create_slider(
            synth_frame, "Click/Noise Level", 0.0, 3.0, 1.0, "click_level",
            "Volume of the high-frequency click/transient layer."
        )
        self.create_slider(
            synth_frame, "Click Decay (ms)", 5.0, 100.0, 10.0, "click_decay",
            "How quickly the click sound fades out. Shorter = snappier."
        )
        self.create_slider(
            synth_frame, "Click Width (Stereo)", 0.0, 2.0, 0.0, "click_width",
            "Stereo width of the click. 0=Mono, >1=Enhanced Stereo."
        )

        # --- Effects Tab ---
        self.create_slider(
            fx_frame, "Saturation Drive (dB)", 0.0, 12.0, 4.5, "drive",
            "Amount of distortion/saturation. Adds harmonics and aggression."
        )
        self.create_slider(
            fx_frame, "Reverb Amount", 0.0, 1.0, 0.0, "reverb",
            "Mix level of the reverb effect for space."
        )
        self.create_slider(
            fx_frame, "Delay Amount", 0.0, 1.0, 0.0, "delay",
            "Mix level of the stereo delay effect."
        )

        # --- Bassline Tab ---
        bass_frame = ttk.LabelFrame(
            bass_frame_tab, text="Bassline & Sidechain (VST/DAW Integration)",
            padding="10"
        )
        ...
        bass_chk = ttk.Checkbutton(
            bass_frame, text="Generate Bassline Loop (138 BPM)",
        ...
        self.create_slider(
            ...
            "Frequency of the generated offbeat bassline."
        )
        # Sidechain Depth
        self.create_slider(
            bass_frame, "Sidechain Depth", 0.0, 1.0, 0.8, "sc_depth",
            "How much the bass volume ducks when the kick hits."
        )

        # Export Trigger Button
        trigger_btn = ttk.Button(
            bass_frame, text="Export Sidechain Trigger (Click)",
